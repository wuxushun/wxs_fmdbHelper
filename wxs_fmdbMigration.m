...
// Public Constants
// 公用常量
NSString *const FMDBMigrationManagerErrorDomain = @"com.layer.wxs_fmdbMigrationManager.errors";
...
BOOL wxs_fmdbIsMigrationAtPath(NSString *path)
{
...
static NSArray *wxs_fmdbClassesConformingToProtocol(Protocol *protocol)
{
...
@implementation wxs_fmdbMigration
...
- (void)addMigration:(id<wxs_fmdbMigrating>)migration
{
...
- (void)addMigrations:(NSArray *)migrations
{
    ...
    for (id<NSObject> migration in migrations) {
        if (![migration conformsToProtocol:@protocol(wxs_fmdbMigrating)]) {
...
- (NSArray *)migrations
{
    ...
    for (NSString *path in migrationPaths) {
        ...
        if ([migrationRegex rangeOfFirstMatchInString:filename options:0 range:NSMakeRange(0, [filename length])].location != NSNotFound) {
            wxs_fmdbFileMigration *migration = [wxs_fmdbFileMigration migrationWithPath:path];
    ...
    if (self.dynamicMigrationsEnabled) {
        NSArray *conformingClasses = wxs_fmdbClassesConformingToProtocol(@protocol(wxs_fmdbMigrating));
        for (Class migrationClass in conformingClasses) {
            if ([migrationClass isSubclassOfClass:[wxs_fmdbFileMigration class]]) continue;
            id<wxs_fmdbMigrating> migration = [migrationClass new];
            [migrations addObject:migration];
        }
    }
...
- (id<wxs_fmdbMigrating>)migrationForVersion:(uint64_t)version
{
    for (id<wxs_fmdbMigrating>migration in [self migrations]) {
        if (migration.version == version) return migration;
    }
    return nil;
}

- (id<wxs_fmdbMigrating>)migrationForName:(NSString *)name
{
    for (id<wxs_fmdbMigrating>migration in [self migrations]) {
...
- (BOOL)migrateDatabaseToVersion:(uint64_t)version progress:(void (^)(NSProgress *progress))progressBlock error:(NSError **)error
{
    ...
    for (NSNumber *migrationVersionNumber in pendingVersions) {
        ...
        id<wxs_fmdbMigrating> migration = [self migrationForVersion:migrationVersion];
        ...
        if (progressBlock) {
            ...
            if (progress.cancelled) {
                ...
                if (error) *error = [NSError errorWithDomain:FMDBMigrationManagerErrorDomain code:wxs_fmdbMigrationManagerErrorMigrationCancelled userInfo:userInfo];
                [self.database rollback];
                break;
            }
        }
...
@interface wxs_fmdbFileMigration ()
@property (nonatomic, readwrite) NSString *name;
@property (nonatomic, readwrite) uint64_t version;
@end

@implementation wxs_fmdbFileMigration
